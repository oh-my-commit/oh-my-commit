...
import "@/styles/commit-messager.css";

provideVSCodeDesignSystem().register(vsCodeButton(), vsCodeTextArea(), vsCodeDivider(), vsCodeCheckbox());
...
const CommitMessage = () => {
    ...
    const handleSubmit = React.useCallback(() => {
        ...
        const commitMessage = state.body ? `${state.title}\n\n${state.body}`.trim() : state.title.trim();

        vscode.postMessage({
            command: "commit", message: commitMessage, isAmendMode: state.isAmendMode,
        });
    }, [state, vscode]);
    ...
    // 只计算选中文件的统计信息
    const stats = React.useMemo(() => state.filesChanged
        .filter((file) => state.selectedFiles.has(file.path))
        .reduce((acc, file) => ({
            additions: acc.additions + file.additions, deletions: acc.deletions + file.deletions,
        }), {additions: 0, deletions: 0}), [state.filesChanged, state.selectedFiles]);
    ...
    return (<div className="commit-container">
            <div className="commit-form">
              ...
              Review and edit the generated commit message
                    ...
                    <vscode-text-area
                        value={state.body}
                        onChange={(e: any) => setState((prev) => ({...prev, body: e.target.value}))}
              ...
              {state.filesChanged.length} files
                    ...
                    <div className="files-tree">
                        <FileTreeErrorBoundary>
                            {fileTree.map((node, index) => (<FileTreeNode
                                    key={`${node.path}-${index}`}
                                    node={node}
                                    level={0}
                ...
                <div className="recent-commits-section">
                    ...
                    <div className="commits-list">
                        {mockRecentCommits.map((commit) => (<div
                                key={commit.hash}
                            ...
                            >
                                ...
                                {hoveredCommit === commit.hash && (<div className="commit-details">
