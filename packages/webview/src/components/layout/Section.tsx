...
const SectionHeader = ({ title, actions }: SectionHeaderProps) => {
  ...
  return (
    <div className="section-header flex items-center justify-between">
      {title && <h1 className="section-title text-base font-medium text-[var(--vscode-editor-foreground)]">{title}</h1>}
...
const SectionFooter = ({ children, className = "" }: SectionFooterProps) => {
  ...
  return <div className={`section-footer flex items-center justify-between ${className}`}>{children}</div>
}
