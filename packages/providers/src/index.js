...
import { APP_ID, APP_NAME, BaseGenerateCommitProvider, OmcStandardModelId, } from "@oh-my-commit/shared/common";
import { GenerateCommitError, } from "@oh-my-commit/shared/common";
import { merge } from "lodash";
import { ResultAsync } from "neverthrow";
import { HttpsProxyAgent } from "https-proxy-agent";
import { formatError } from "@oh-my-commit/shared/common";
const loadPrompt = (lang, diff) => {
    // todo: parse from `commit-prompt.hbs`
    return `作为一个经验丰富的开发者，请分析以下代码变更并生成提交信息：\n${diff}`;
};
class OmcStandardModel {
...
export class OmcProvider extends BaseGenerateCommitProvider {
    ...
    generateCommit(input) {
        ...
        return (
        // 1. call api
        ResultAsync.fromPromise(this.callApi(input), (error) => new GenerateCommitError(-10086, `failed to call api, reason: ${formatError(error)}`))
            // 2. handle api result
            .andThen((response) => ResultAsync.fromPromise(this.handleApiResult(response), (error) => new GenerateCommitError(-10087, `failed to handle api result, reason: ${formatError(error)}`)))
            // 3. add metadata
            .map((result) => {
            merge(result.meta, {
