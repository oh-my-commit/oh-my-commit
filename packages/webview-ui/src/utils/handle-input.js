...
exports.handleKeyDown = exports.getPreviousLineIndentLevel = exports.getIndentationLevel = void 0;
...
const getPreviousLineIndentLevel = (text, currentLineStart) => {
    const lastNewline = text.lastIndexOf("\n", currentLineStart - 1);
    if (lastNewline === -1)
        return 0;
    const previousLine = text.slice(text.lastIndexOf("\n", lastNewline - 1) + 1, lastNewline);
    return (0, exports.getIndentationLevel)(previousLine);
};
exports.getPreviousLineIndentLevel = getPreviousLineIndentLevel;
// Handle keydown events for the markdown editor
const handleKeyDown = (e, text, selectionStart, selectionEnd, onChange, setSelectionRange) => {
    ...
    if (e.key === "Tab") {
        ...
        for (let i = startLineIndex; i <= endLineIndex; i++) {
            ...
            if (isList && !e.shiftKey) {
                const prevIndentLevel = i === 0
                    ? (0, exports.getPreviousLineIndentLevel)(text, text.indexOf(line))
                    : (0, exports.getIndentationLevel)(lines[i - 1]);
                console.log("List item indentation check:", {
                    prevIndentLevel,
                    currentLevel: (0, exports.getIndentationLevel)(line),
                });
                // Only allow indenting one level deeper than the previous line
                if ((0, exports.getIndentationLevel)(line) > prevIndentLevel) {
        ...
        const beforeStartLine = lines.slice(0, startLineIndex).join("\n").length + (startLineIndex > 0 ? 1 : 0);
        ...
        setTimeout(() => {
            if (startLineIndex === endLineIndex) {
                ...
                console.log("Setting single line cursor position:", {
                    ...
                    oldEnd: selectionEnd,
                    newStart: newPos,
                    newEnd: selectionStart === selectionEnd ? newPos : newPos + (selectionEnd - selectionStart),
                });
                setSelectionRange(newPos, selectionStart === selectionEnd ? newPos : newPos + (selectionEnd - selectionStart));
            }
            else {
...
const handleKeyDownOriginal = (e, onValueChange) => {
    ...
    if (e.currentTarget instanceof HTMLTextAreaElement) {
        ...
        if (e.key === "Tab") {
            ...
            for (let i = startLineIndex; i <= endLineIndex; i++) {
                const line = lines[i];
                const isList = line.trim().startsWith("- ");
                const currentIndent = (0, exports.getIndentationLevel)(line);
